...
MLFLOW_AUTH_DATABASE_URI="${MLFLOW_AUTH_DATABASE_URI:?MLFLOW_AUTH_DATABASE_URI must be set}"
MLFLOW_BACKEND_STORE_URI="postgresql+psycopg2://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:${POSTGRES_PORT}/${POSTGRES_DB}"

if [[ "${MLFLOW_AUTH_DATABASE_URI}" != postgresql+psycopg2://* ]]; then
  echo "ERROR: MLFLOW_AUTH_DATABASE_URI must use postgresql+psycopg2://"
  exit 1
fi
